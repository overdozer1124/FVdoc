...
<div class="fvdoc-section">
  ...
  <div class="fvdoc-hint">※ 記号は自動変換：。→︒　、→︑　（）→︵︶　ー→｜　[]→﹇﹈　:→‥</div>
</div>
...
.fvdoc-field input, .fvdoc-field select {
...
.fvdoc-field input:focus, .fvdoc-field select:focus {
  border-color: #1a73e8; box-shadow: 0 0 0 2px rgba(26,115,232,0.12);
}
#fvdoc-preview-box {
...
.fvdoc-table-item {
  ...
  margin-bottom: 8px; font-size: 12px; color: #202124; overflow: hidden;
}
.fvdoc-table-item-top { padding: 8px 10px; }
.fvdoc-table-item-preview { font-weight: 500; }
.fvdoc-table-item-meta    { color: #5f6368; font-size: 11px; margin-top: 2px; }
.fvdoc-table-item-actions { display: flex; gap: 0; border-top: 1px solid #e8eaed; }
...
#fvdoc-justify-cols {
  ...
  border: 1px solid #e8eaed; border-radius: 4px; margin-bottom: 8px;
}
...
.fvdoc-justify-panel-btns { display: flex; gap: 6px; }
  `;
}
...
function openJustifyPanel(tableInfo, numCols) {
  ...
  for (const l of lines) {
    for (let i = 0; i < l.length; i += cpLine)
      chunks.push(l.substring(i, i + cpLine));
  }
...
async function onApplyJustify() {
  ...
  if (!checked.length) { showStatus('err', '列を1つ以上選択してください。'); return; }
