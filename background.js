...
async function setPBBBeforeTable(token, docId, tableStartIndex) {
  ...
  console.log('[FVdoc v4] setPBBBeforeTable: auto_para に pbb=true を設定 (startIndex=', prevEl.startIndex, ')');
  ...
  const verifyTableIdx = verifyContent.findIndex(el => el.table && el.startIndex === tableStartIndex);
  const surroundingEls = verifyContent.slice(Math.max(0, verifyTableIdx - 2), verifyTableIdx + 1).map(el => ({
    type:       el.table ? 'TABLE' : 'PARA',
    startIndex: el.startIndex,
    endIndex:   el.endIndex,
    pbb:        el.paragraph?.paragraphStyle?.pageBreakBefore
  }));
  console.log('[FVdoc v4] setPBBBeforeTable 検証 (TABLE直前2要素+TABLE):', JSON.stringify(surroundingEls));
}
